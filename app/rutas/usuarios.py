...
def listar_usuarios():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "usuarios": [usuario.to_dict() for usuario in usuarios]}
        )
...
def crear_usuario():
    ...
    try:
        ...
        if not all([nombre_usuario, nombres, apellidos, email]):
            return jsonify(
                {
                    "success": False,
                    "error": "Faltan parámetros requeridos: nombre_usuario, nombres, apellidos, email",
                }
            ), 400

        usuario_id = Usuario.crear_usuario(
            nombre_usuario, nombres, apellidos, email, nivel
        )
        ...
        return jsonify({"success": False, "error": "Error creando usuario"}), 500
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
@bp_usuarios.route("/<int:usuario_id>/insignias", methods=["GET"])
...
def obtener_logros_usuario(usuario_id):
    """Obtener logros (insignias) de un usuario - alias para insignias"""
    try:
        insignias = Usuario_Insignia.obtener_insignias_usuario(usuario_id)
        return jsonify({"success": True, "logros": insignias})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500


@bp_usuarios.route("/<int:usuario_id>/ranking", methods=["GET"])
def obtener_ranking_usuario(usuario_id):
    """Obtener rankings de un usuario"""
    try:
        rankings = Ranking.obtener_ranking_usuario(usuario_id)
        return jsonify(
            {"success": True, "rankings": [ranking.to_dict() for ranking in rankings]}
        )
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def obtener_estadisticas_usuario(usuario_id):
    ...
    try:
        estadisticas = Usuario.obtener_estadisticas(usuario_id)
        if estadisticas:
...
@bp_usuarios.route("/<int:usuario_id>/notificaciones", methods=["GET"])
...
@bp_usuarios.route(
    "/<int:usuario_id>/notificaciones/<int:notificacion_id>/leer",
    methods=["PUT"],
)
...
@bp_usuarios.route("/<int:usuario_id>/exportar/sheets", methods=["POST"])
def exportar_resultados_sheets(usuario_id):
    ...
    try:
        ...
        return jsonify(
            {"success": False, "error": "Error exportando a Google Sheets"}
        ), 500
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500


@bp_usuarios.route("/<int:usuario_id>/exportar/excel", methods=["GET"])
def exportar_resultados_excel(usuario_id):
    ...
    try:
        ...
        return jsonify({"success": False, "error": "Error exportando a Excel"}), 500
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def actualizar_nivel_usuario(usuario_id):
    ...
    try:
        ...
        if not nuevo_nivel:
            return jsonify({"success": False, "error": "Nivel requerido"}), 400

        success = Usuario.actualizar_nivel(usuario_id, nuevo_nivel)

        if success:
            return jsonify(
                {"success": True, "message": "Nivel actualizado exitosamente"}
            )
        return jsonify({"success": False, "error": "Error actualizando nivel"}), 500
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
