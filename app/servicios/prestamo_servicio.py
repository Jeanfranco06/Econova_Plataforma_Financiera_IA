...
class ServicioPrestamo:
    ...
    @staticmethod
    def calcular_tabla_amortizacion(monto: float, tasa_anual: float, 
                                   plazo_meses: int) -> List[Dict[str, Any]]:
        """
    ...
    @staticmethod
    def calcular_prestamo_completo(monto: float, tasa_anual: float, 
                                   plazo_meses: int, tasa_impuesto: float = 0,
                                   comision_inicial: float = 0) -> Dict[str, Any]:
        """
        ...
        cuota_mensual = ServicioPrestamo.calcular_cuota_mensual(monto, tasa_anual, plazo_meses)
        tabla_amortizacion = ServicioPrestamo.calcular_tabla_amortizacion(monto, tasa_anual, plazo_meses)
    ...
    @staticmethod
    def analizar_sensibilidad_prestamo(monto: float, tasa_anual: float, 
                                       plazo_meses: int, variacion_tasa: float = 0.5) -> Dict[str, Any]:
        """
        ...
        cuota_base = ServicioPrestamo.calcular_cuota_mensual(monto, tasa_anual, plazo_meses)
        ...
        for tasa in tasas_analiticas:
            ...
            try:
                cuota = ServicioPrestamo.calcular_cuota_mensual(monto, tasa, plazo_meses)
        ...
        return {
            ...
            'impacto_maximo_alza': round((max([e['variacion_cuota_porcentaje'] for e in escenarios if e['tasa'] > tasa_anual], 
                                             default=0)), 2),
            'impacto_maximo_baja': round((min([e['variacion_cuota_porcentaje'] for e in escenarios if e['tasa'] < tasa_anual],
                                             default=0)), 2)
        }
    
    @staticmethod
    def comparar_plazos(monto: float, tasa_anual: float, plazos: List[int]) -> Dict[str, Any]:
        """
        ...
        for plazo in plazos:
            try:
                cuota = ServicioPrestamo.calcular_cuota_mensual(monto, tasa_anual, plazo)
                ...
                comparativa.append({
                    ...
                    'costo_interes': round(costo_interes, 2),
                    'costo_total': round(costo_total, 2),
                    'ahorro_vs_menor_plazo': round(costo_interes, 2) if plazo == plazos[0] else 'N/A'
                })
            except:
                continue
        ...
        return {
            'monto': monto,
            'tasa': tasa_anual,
            'comparativa_plazos': comparativa
        }
