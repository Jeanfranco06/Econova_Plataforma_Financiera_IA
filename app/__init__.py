...
from app.utils.base_datos import DatabaseManager, verificar_conexion
import os
...
def crear_app(config_name='development'):
    ...
    def index():
        return jsonify({
            'message': 'Bienvenido a Econova - Plataforma Inteligente de Simulación Financiera',
            'version': app.config['API_VERSION'],
            'endpoints': {
                'financiero': f"{app.config['API_PREFIX']}/financiero",
                'usuarios': f"{app.config['API_PREFIX']}/usuarios",
                'ml': f"{app.config['API_PREFIX']}/ml",
                'chatbot': f"{app.config['API_PREFIX']}/chatbot",
                'benchmarking': f"{app.config['API_PREFIX']}/benchmarking"
            }
        }), 200
