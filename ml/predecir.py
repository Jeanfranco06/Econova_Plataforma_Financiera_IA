...
def analisis_tornado(
    ...
    if SERVICIOS_DISPONIBLES:
        ...
        return _analisis_tornado_simple(
            inversion_inicial, flujos_caja, tasa_descuento, variacion
        )
...
def analisis_escenarios(
    ...
    if SERVICIOS_DISPONIBLES:
        ...
        return _analisis_escenarios_simple(
...
def _predecir_ingresos_simple(datos: Dict[str, float]) -> Dict[str, Any]:
    ...
    margen = (datos["ingresos_anuales"] - datos["gastos_operativos"]) / datos[
        "ingresos_anuales"
    ]
    ratio_deuda = datos["pasivos_totales"] / datos["activos_totales"]
    roi = (datos["ingresos_anuales"] - datos["gastos_operativos"]) / datos[
        "activos_totales"
    ]
    ...
    # Calcular intervalos de confianza aproximados (±15% de la predicción)
    limite_inferior = prediccion * 0.85
    limite_superior = prediccion * 1.15

    return {
        "ingresos_predichos": round(prediccion, 2),
        "crecimiento_esperado_pct": round(crecimiento * 100, 2),
        "limite_inferior_95": round(limite_inferior, 2),
        "limite_superior_95": round(limite_superior, 2),
...
def _clasificar_riesgo_simple(datos: Dict[str, float]) -> Dict[str, Any]:
    ...
    # Calcular probabilidad basada en el score
    probabilidad = score / 8.0  # Score máximo es 8

    # Generar recomendación según el nivel
    recomendaciones = {
        "Bajo": "Su empresa presenta una sólida posición financiera. Continúe monitoreando los indicadores clave.",
        "Medio": "Se recomienda mejorar el margen operativo y optimizar el nivel de endeudamiento.",
        "Alto": "Se requiere atención inmediata: considere reestructurar deuda y reducir gastos operativos.",
    }
...
def _simular_monte_carlo_simple(
    inv, flujos, tasa, var_flujos, var_tasa, n_sim
) -> Dict[str, Any]:
...
def _analisis_escenarios_simple(inv, flujos, tasa) -> Dict[str, Any]:
    ...
    return {
        "escenarios": {
            "pesimista": {
                "van": round(calc_van(inv, [f * 0.75 for f in flujos], tasa * 1.25), 2)
            },
            "base": {"van": round(calc_van(inv, flujos, tasa), 2)},
            "optimista": {
                "van": round(calc_van(inv, [f * 1.25 for f in flujos], tasa * 0.85), 2)
            },
        }
    }
...
if __name__ == "__main__":
    ...
    print(
        f"   Crecimiento esperado: {resultado_ingresos['crecimiento_esperado_pct']:.2f}%"
    )
    ...
    print(
        f"   Crecimiento anual: {resultado_crecimiento['crecimiento_porcentaje']:.2f}%"
    )
